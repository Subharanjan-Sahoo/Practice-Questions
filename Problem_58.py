...
import sys
def fuelComsumption(input1,input2):
    if input1 <= 0:
        print(input1 , "is an Invalid Input") 
        sys.exit()
    if input2 <= 0:
        print(input2 , "is an invalid input")   
        sys.exit() 
    fuelComsumptionis = (input1/input2)*100
    convertToMiles = input2 * 0.6214
    convertToGallons = input1 * 0.2642
    MilesGallons = convertToMiles/convertToGallons 

    print(round(fuelComsumptionis,2))
    print(round(MilesGallons,2))



input1 = int(input("Enter the no of liters to fill the tank: "))
input2 = int(input("Enter the distance covered: "))
fuelComsumption(input1,input2)